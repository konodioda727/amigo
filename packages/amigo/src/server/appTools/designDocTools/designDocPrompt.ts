...
7. 资产规则：创建、整体重建或局部更新设计稿前，先调用 listDesignAssets 查看当前已存储的设计资产。设计稿里只能复用 listDesignAssets 已返回的资产；component 用 <use component="...">，图片用 <img asset="...">。不要在这一步临时创建新资产、内联补资产或绕过资产系统直接画替代图形。不要使用 SVG，包括内联 <svg>、path、多边形或手写矢量图标。
8. 文案规则：设计稿中的标题、按钮、标签、正文和辅助说明里都禁止使用 emoji。
9. 只使用工具 description 里允许的标签和样式属性；不要写 class、脚本或外部样式表。当前输出的是静态设计稿，不需要动画、过渡或复杂交互逻辑；hover/focus/active 之类状态如果必须表达，只会被当作附加元数据透传，不参与布局计算。
10. 创建页面时必须显式写 <page width="...">，宽度要匹配目标端。移动端不要沿用 1440 桌面宽度；手机稿优先使用 375、390、393、414，平板稿使用 768、810、834，桌面稿再使用 1280、1440。
11. 设计稿尺寸必须受控。页面宽度保持在目标端真实范围内，页面高度、section 高度、节点宽高都要是合理的设计尺寸；不要生成超大画布、超长 section 或离谱节点尺寸。
`.trim();
...
9. 如果要复用设计资产，只能使用 listDesignAssets 已返回的已存储资产。component 使用 <use component="asset-id" id="instance-id" />，图片使用 <img asset="asset-id" />。不要临时创建新资产，不要使用 SVG，包括内联 <svg> 或任何手写矢量图标。
10. 文案、按钮标签、徽标文案和说明文字里禁止使用 emoji。
11. 样式可以写在 inline style，也可以直接写成展示属性，如 background、padding、display、font-size、flex。
12. section 的 name 必须是简短的区块名，不要用整段正文、关键词堆砌或自动摘取的文案。
13. 不要写 class、<style> 或脚本；长度使用 number、px 或百分比。当前是静态设计稿，不需要动效或复杂交互；如果必须表达 hover/focus/active，只会作为元数据透传，不会参与布局或动画渲染。
14. 必须显式设置 <page width="...">；如果任务是移动端页面，使用手机画布宽度而不是 1440。
15. 所有 section 和主要节点都要保持合理宽高。优先给主要卡片、图片、按钮、容器写明确尺寸或受控约束，避免生成超大宽高值。
`.trim();
