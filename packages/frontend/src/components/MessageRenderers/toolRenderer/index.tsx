...
export type ToolRendererProps<T extends ToolNames> = FrontendToolMessageType<T> & {
  updateTime?: number;
};
type ToolRenderers = {
      ...
      <div className="flex justify-center w-full mb-3">
        <div className="flex items-center gap-2 px-3 py-2 bg-error/10 rounded-lg text-error text-xs max-w-[80%]">
          ...
          <span className="whitespace-pre-wrap">
            工具调用失败：{toolName} - {error}
          </span>
          {updateTime && (
            <span className="opacity-50 ml-1">{new Date(updateTime).toLocaleTimeString()}</span>
          )}
        </div>
      </div>
    );
    ...
    <div className="mb-4 max-w-[80%]">
      <div className="chat chat-start">
        <div className="chat-bubble bg-neutral-100 text-neutral-900 rounded-xl px-4 py-3">
          ...
          {toolOutput && (
            <div className="mt-2 text-xs text-success">输出: {JSON.stringify(toolOutput)}</div>
          )}
          {updateTime && (
