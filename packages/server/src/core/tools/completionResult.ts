...
export const CompletionResult = createTool({
  ...
  whenToUse:
    ...
    "**🚨 重要前置条件：**\n" +
    "- ⚠️ **在调用此工具前，必须确认 taskList 中的任务都已完成或有明确收尾结论**\n" +
    "- ⚠️ 若刚调用了 `executeTaskList`，请等待异步推送的执行结果后再判断是否收尾\n" +
  ...
  params: [
    {
      name: "result",
      optional: false,
      description: "最终返回给用户的完整结论（Markdown）",
    },
  ],
  async invoke({ params }) {
    const finalAnswer = params.result;
    logger.info("任务完成，最终结论：", finalAnswer);
