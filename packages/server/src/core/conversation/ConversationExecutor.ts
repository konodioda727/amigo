...
export class ConversationExecutor {
  ...
  async execute(conversation: Conversation): Promise<void> {
    ...
    // 等待用户输入
    await pWaitFor(
      () => !!conversation.userInput || conversation.isAborted || conversation.status === "aborted",
    );
  ...
  private async handlePendingToolConfirmation(conversation: Conversation): Promise<void> {
    ...
    if (conversation.type === "sub") {
      ...
      if (conversation.isAborted || conversation.status === "aborted") {
        logger.info("[ConversationExecutor] 子任务工具执行后检测到中断，停止 loop");
        return;
      }
      ...
      if (shouldContinue && !conversation.isAborted) {
        logger.info(`[ConversationExecutor] 子任务工具执行完毕，继续执行 loop`);
        return this.executeLoop(conversation);
      }

      logger.info(`[ConversationExecutor] 子任务工具执行完毕，停止 loop`);
      return;
    }
    ...
    if (input === "confirm") {
      ...
      if (conversation.isAborted || conversation.status === "aborted") {
        logger.info("[ConversationExecutor] 工具执行后检测到中断，停止 loop");
        return;
      }
  ...
  private async executeLoop(conversation: Conversation): Promise<void> {
    ...
    try {
      ...
      if (shouldContinue && !conversation.isAborted && conversation.status !== "aborted") {
