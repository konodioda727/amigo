  ...
  try {
    ...
    const sessionHistoriesPromises = taskDirs
      ...
      .map(async (dirent) => {
        ...
        const originalJsonPath = path.join(globalStoragePath, taskId, `${StorageType.ORIGINAL}.json`);
        ...
        try {
          ...
          // 从第一条用户消息中提取标题
          const firstUserMessage = frontendData.messages?.find((msg: any) => msg.type === 'userSendMessage');
          const title = firstUserMessage?.data?.message?.substring(0, 50) || '未命名对话';
          const updatedAt = frontendData.updatedAt || originalData.updatedAt || new Date().toISOString();
