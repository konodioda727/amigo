...
import { DESIGN_DOC_V3_SYSTEM_PROMPT_APPENDIX } from "./designDocPrompt";
import { editDesignDocTool, readDesignDocTool } from "./designDocs";
...
function buildCheckSteps(
  ...
  if (preset === "quick") {
    ...
    return keys.map((key) => step(key)).filter((v) => !!v);
  }
...
5. 设计稿必须使用可执行的 v3 schema，根字段只保留：page -> designTokens -> sections。
6. 在填写 sections 时，先定每个 section 的 y、height、layout，再补 nodes；每个 node 都要具备最终可落地的几何信息和内容信息，不能把布局留给后续推断。
7. 如果页面已经有设计稿，默认先 readDesignDoc；read 成功后，不要立刻整份重写。
8. 先判断这次需求属于局部修改还是整稿重写。
9. 若只是局部调整，必须优先使用 editDesignDoc 的 startLine/endLine 做局部替换，不要每次整份重写。
10. 只有在页面结构大改、区块顺序重排、或整体视觉重构时，才考虑整份重写 design doc。
11. 如果判断必须整稿重写，先向用户说明原因并征求意见；未经用户明确同意，不要整份重写设计稿。
12. 如果是已有页面迭代，优先在现有设计稿基础上更新，保持 pageId、section id、node id 稳定。
13. 修改代码后必须使用 runChecks 进行验证（至少 quick；必要时 all）。
14. 优先使用 editFile 的 patch 模式进行小范围修改，避免不必要的大范围重写。
15. 如果验证失败，先阅读失败输出并定位原因，再进行下一轮修复。
16. 最终结果中要明确说明：设计稿处理情况、代码修改内容、验证结果、剩余风险/未覆盖项。
