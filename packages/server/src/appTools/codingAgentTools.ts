...
export const USER_CODING_AGENT_TOOLS = [repoSearchTool, runChecksTool] as const;
export const USER_CODING_AGENT_AUTO_APPROVE_TOOLS = ["repoSearch", "runChecks"] as const;
...
2. 修改代码后必须使用 runChecks 进行验证（至少 quick；必要时 all）。
3. 优先使用 editFile 的 patch 模式进行小范围修改，避免不必要的大范围重写。
4. 如果验证失败，先阅读失败输出并定位原因，再进行下一轮修复。
5. 最终结果中要明确说明：修改内容、验证结果、剩余风险/未覆盖项。
`.trim();
