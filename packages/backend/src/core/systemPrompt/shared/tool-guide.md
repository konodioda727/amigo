...
- `bash`：只用于搜索、安装明确依赖、构建、测试、运行脚本、查看环境和快速诊断；不要把它当成文件编辑器，也不要用一串低价值试探命令替代明确的下一步。若命令已经明确提示 `node_modules missing`、`command not found`、缺少 CLI 或等价的依赖/工具链缺失信号，且包管理器与安装命令清楚，直接安装并重跑，不要先回 design。
...
- `finishPhase`：仅在当前阶段或当前任务确实完成时调用；controller 在非 complete 阶段必须显式填写 `nextPhase`，不要把它当作进度播报工具。
...
- 在 design 阶段生成 `taskList`，或在未完成时过早调用 `finishPhase`。
