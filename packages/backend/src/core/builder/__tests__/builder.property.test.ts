...
describe("构建器属性测试", () => {
  ...
  describe("属性 9: 构建器链式调用", () => {
    ...
    test("sandboxManager() 方法应返回相同的构建器实例", () => {
      const builder = new AmigoServerBuilder();
      const result = builder.sandboxManager(mockSandboxManager);
      expect(result).toBe(builder);
    });

    test("baseTools() 方法应返回相同的构建器实例", () => {
      const builder = new AmigoServerBuilder();
      const result = builder.baseTools({ main: [createMockTool("main_tool")] });
      expect(result).toBe(builder);
    });

    test("systemPrompts() 方法应返回相同的构建器实例", () => {
      const builder = new AmigoServerBuilder();
      const result = builder.systemPrompts({ main: "main prompt", sub: "sub prompt" });
      expect(result).toBe(builder);
    });
