...
const getContextAnchorIndex = (messages: ChatMessage[]): number | null => {
  const index = messages.findLastIndex(
    (message) => message.type === "compaction" || message.type === "checkpoint",
  );
...
const getMessagesForCurrentContext = (messages: ChatMessage[]): ChatMessage[] => {
  ...
  const anchorIndex = getContextAnchorIndex(messages);
  if (anchorIndex === null) {
    return [...messages];
  }
...
const getCompletionSeedState = (
  conversation: Conversation,
): NonNullable<Conversation["workflowState"]["completionSeedState"]> | null =>
  conversation.workflowState?.completionSeedState ||
  conversation.memory.workflowState?.completionSeedState ||
  null;

const getRawHistoryStartIndex = (
  messages: ChatMessage[],
  completionSeedState: NonNullable<Conversation["workflowState"]["completionSeedState"]> | null,
): number => {
  if (!completionSeedState) {
    return 0;
  }
  return Math.min(Math.max(completionSeedState.sourceMessageCount, 0), messages.length);
};

const buildMessagesForModelContext = (
  conversation: Conversation,
  ephemeralMessages: ChatMessage[] = [],
): {
  rawHistoryStartIndex: number;
  rawMessages: ChatMessage[];
  selectedMessages: ChatMessage[];
} => {
  const completionSeedState = getCompletionSeedState(conversation);
  const rawHistoryStartIndex = getRawHistoryStartIndex(
    conversation.memory.messages,
    completionSeedState,
  );
  const rawMessages = conversation.memory.messages.slice(rawHistoryStartIndex);
  const selectedMessages = [
    ...(completionSeedState?.messages || []),
    ...getMessagesForCurrentContext(rawMessages),
    ...ephemeralMessages,
  ];

  return {
    rawHistoryStartIndex,
    rawMessages,
    selectedMessages,
  };
};
...
const decideCompressionSplit = (
  ...
  const anchorIndex = getContextAnchorIndex(messages) ?? 0;
...
const buildContextUsageSnapshot = (
  ...
  const selectedMessages = [
    ...buildMessagesForModelContext(conversation, ephemeralMessages).selectedMessages,
  ];
...
export class ContextCompressionManager {
  ...
  async prepareMessages(
    ...
    if (!snapshot) {
      ...
      const selectedMessages = buildMessagesForModelContext(
        conversation,
    ...
    let contextMessages = buildMessagesForModelContext(conversation, mergedEphemeralMessages);
    let selectedMessages = contextMessages.selectedMessages;
    ...
    try {
      ...
      conversation.memory.insertMessageAt(
        contextMessages.rawHistoryStartIndex + split.keepStartIndex,
        {
          role: "user",
          type: "compaction",
      ...
      contextMessages = buildMessagesForModelContext(conversation, mergedEphemeralMessages);
      selectedMessages = contextMessages.selectedMessages;
